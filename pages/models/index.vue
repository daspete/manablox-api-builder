    ...
    <div>
        <panel title="Datamodels">
            <template slot="header">
                <v-btn icon nuxt to="models/create">
                ...
                >
                    ...
                    <template slot="items" slot-scope="props">
                        <tr :active="props.selected">
                            ...
                            <td class="text-xs-left">{{ props.item.model_name }}</td>
                            <td class="text-xs-left">{{ props.item.model_slug }}</td>
                            <td class="text-xs-left">{{ props.item.model_description }}</td>
...
export default {
    ...
    async asyncData({ app }){
        ...
        try{
            models = await app.$axios.$get('/api/v1/models');
    ...
    methods: {
        ...
        async OnDeleteItemConfirm(){
            ...
            try {
                await this.$axios.$delete(`/api/v1/models/${ this.current.id }`);
            }catch(e){
                console.log(e);
            }
